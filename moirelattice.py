...
def calculateMixedMoireComponents(lattice1, lattice2, a1, a2, theta_degrees, number_of_components = 3):
	"""
	calculate the longest first-shell moire components for a
	square-hexagonal lattice pair
	...
	if lattice1 == lattice2:
		raise ValueError("Mixed components require one square and one hexagonal lattice.")

	if lattice1 not in ("Hexagonal", "Square"):
		raise ValueError("Layer 1 must be Hexagonal or Square.")

	if lattice2 not in ("Hexagonal", "Square"):
		raise ValueError("Layer 2 must be Hexagonal or Square.")
	...
	def createFirstShell(lattice_type, lattice_constant):
		...
		used by hexatoms.py and squareatoms.py
		"""
